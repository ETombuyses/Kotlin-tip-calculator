...
// specify with which runner the test must run
@RunWith(AndroidJUnit4::class)
class CalculatorTests {
    // import the activity
    @get:Rule() // indicates that the following rule must execute before each test of the class
    val activity = ActivityScenarioRule(MainActivity::class.java)

    @Test
    fun calculate_20_percent_tip() {
        onView(withId(R.id.cost_of_service_text))
    ...
    fun roundTip() {
        ...
        onView(withId(R.id.tip_result)).check(
            matches(withText(containsString("$8.00")))
        )
    }
}